...
def default_config_path() -> Path:
    configured = os.environ.get("DOCKERNAUT_CONFIG")
    if configured:
        return Path(configured).expanduser()
    return Path.home() / ".config" / "dockernaut" / "targets.toml"
...
def load_config(path: str | Path | None = None) -> Config:
    ...
    for name, table in target_tables.items():
        ...
        invalid = [key for key, value in table.items() if key not in _RESERVED and not isinstance(value, dict)]
        if invalid:
    ...
    cache = Path(raw.get("cache_dir", "~/.cache/dockernaut")).expanduser()
