...
_KEYSYMS = {
    ...
    "pagedown": 0xFF56, "end": 0xFF57, "insert": 0xFF63, "shift": 0xFFE1,
    "ctrl": 0xFFE3, "control": 0xFFE3, "alt": 0xFFE9, "meta": 0xFFE7,
    "super": 0xFFEB, "space": 0x20,
}
...
def _vnc_response(password: str, challenge: bytes) -> bytes:
    key = bytes(_reverse_bits(byte) for byte in password.encode("latin1")[:8].ljust(8, b"\0"))
    process = subprocess.run(
        ["openssl", "enc", "-des-ecb", "-K", key.hex(), "-nosalt", "-nopad", "-provider", "legacy", "-provider", "default"],
        input=challenge,
        ...
        while True:
            ...
            if message == 0:
                ...
                for _ in range(rectangles):
                    x, y, width, height, encoding = struct.unpack(">HHHHi", _exact(self.sock, 12))
                    if encoding == -223:
                    ...
                    for row in range(height):
                        ...
                        for column in range(width):
                            blue, green, red = pixels[source + column * 4:source + column * 4 + 3]
                            offset = target + column * 3
                            rgb[offset:offset + 3] = bytes((red, green, blue))
                return Frame(encode_png(self.width, self.height, bytes(rgb)), self.width, self.height, "vnc")
            if message == 1:
...
class VNCAdapter(Adapter):
    ...
    def _connect(self) -> RFB:
        return RFB(str(self.config["host"]), int(self.config.get("port", 5900)), self.config.get("password"), float(self.config.get("timeout", 10)))
    ...
    def _act_sync(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            if action in {"move", "click", "double_click", "right_click"}:
                ...
                self.pointer = target
                if action != "move":
                    button = 4 if action == "right_click" else 1 << (int(params.get("button", 1)) - 1)
            ...
            if action == "scroll":
                target = (int(params["x"]), int(params["y"])) if "x" in params else self.pointer
