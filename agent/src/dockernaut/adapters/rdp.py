...
_VIRTUAL_KEYS = {
    "backspace": "VK_BACK", "escape": "VK_ESCAPE", "tab": "VK_TAB", "return": "VK_RETURN",
    "enter": "VK_RETURN", "insert": "VK_INSERT", "delete": "VK_DELETE", "home": "VK_HOME",
    "end": "VK_END", "pageup": "VK_PRIOR", "pagedown": "VK_NEXT", "left": "VK_LEFT",
    "up": "VK_UP", "right": "VK_RIGHT", "down": "VK_DOWN", "ctrl": "VK_CONTROL",
    "control": "VK_CONTROL", "alt": "VK_MENU", "shift": "VK_SHIFT", "space": "VK_SPACE",
}


class RDPAdapter(Adapter):
    kind = "rdp"
    capabilities = frozenset({Capability.CAPTURE, Capability.POINTER, Capability.KEYBOARD})
    ...
    async def available(self) -> tuple[bool, str | None]:
        try:
            ...
            return False, "install dockernaut-agent[rdp] for RDP support"
    ...
    async def capture(self) -> Frame:
        ...
        async with self._lock:
            ...
            deadline = asyncio.get_running_loop().time() + float(self.config.get("capture_timeout", 10))
    ...
    async def act(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            if action in {"move", "click", "double_click", "right_click"}:
                ...
                error = None
                for x, y in points:
                    _, error = await connection.send_mouse(MOUSEBUTTON.MOUSEBUTTON_HOVER, x, y, False)
                    if error:
                        raise ActionError(str(error))
                    if delay:
                ...
                self.pointer = target
                if action != "move":
                    button = MOUSEBUTTON.MOUSEBUTTON_RIGHT if action == "right_click" else MOUSEBUTTON.MOUSEBUTTON_LEFT
                    for _ in range(2 if action == "double_click" else 1):
                        _, error = await connection.send_mouse(button, *target, True)
                        if not error:
                            _, error = await connection.send_mouse(button, *target, False)
                        if error:
                            raise ActionError(str(error))
            ...
            if action == "drag":
                ...
                for point in approach:
                    _, error = await connection.send_mouse(MOUSEBUTTON.MOUSEBUTTON_HOVER, *point, False)
                    if error:
                        raise ActionError(str(error))
                    if approach_delay:
                        await asyncio.sleep(approach_delay)
                _, error = await connection.send_mouse(MOUSEBUTTON.MOUSEBUTTON_LEFT, *start, True)
                if error:
                    raise ActionError(str(error))
                points, duration = trajectory(start, end, params.get("duration_ms"))
                delay = duration / max(1, len(points)) / 1000
                for point in points:
                    _, error = await connection.send_mouse(MOUSEBUTTON.MOUSEBUTTON_LEFT, *point, True)
                    if error:
                        raise ActionError(str(error))
                    if delay:
                        await asyncio.sleep(delay)
                _, error = await connection.send_mouse(MOUSEBUTTON.MOUSEBUTTON_LEFT, *end, False)
                if error:
                    raise ActionError(str(error))
                self.pointer = end
                return {"from": list(start), "to": list(end)}
            if action == "scroll":
                button = MOUSEBUTTON.MOUSEBUTTON_WHEEL_UP if params.get("direction", "down") == "up" else MOUSEBUTTON.MOUSEBUTTON_WHEEL_DOWN
                x, y = int(params.get("x", 0)), int(params.get("y", 0))
                for _ in range(max(0, int(params.get("amount", 3)))):
                    _, error = await connection.send_mouse(button, x, y, False, 120)
                    if error:
                        raise ActionError(str(error))
                return {"direction": params.get("direction", "down"), "amount": int(params.get("amount", 3))}
            if action == "type":
