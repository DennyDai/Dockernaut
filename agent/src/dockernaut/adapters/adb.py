...
_KEYEVENTS = {
    "backspace": "KEYCODE_DEL", "delete": "KEYCODE_FORWARD_DEL", "enter": "KEYCODE_ENTER",
    "return": "KEYCODE_ENTER", "escape": "KEYCODE_ESCAPE", "home": "KEYCODE_HOME",
    "back": "KEYCODE_BACK", "tab": "KEYCODE_TAB", "space": "KEYCODE_SPACE",
    "up": "KEYCODE_DPAD_UP", "down": "KEYCODE_DPAD_DOWN", "left": "KEYCODE_DPAD_LEFT",
    "right": "KEYCODE_DPAD_RIGHT", "pageup": "KEYCODE_PAGE_UP", "pagedown": "KEYCODE_PAGE_DOWN",
}


class ADBAdapter(Adapter):
    kind = "adb"
    capabilities = frozenset({Capability.CAPTURE, Capability.POINTER, Capability.KEYBOARD, Capability.SHELL, Capability.VIEWER})
    ...
    async def _checked(self, *args: str) -> bytes:
        ...
        if code:
            raise ActionError(stderr.decode(errors="replace").strip() or f"adb exited {code}")
        return stdout
    ...
    async def available(self) -> tuple[bool, str | None]:
        ...
        return (True, None) if code == 0 and stdout.strip() == b"device" else (False, stderr.decode(errors="replace").strip() or "ADB device unavailable")
    ...
    async def act(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "drag":
            ...
            await self._checked("shell", "input", "swipe", *map(str, values), str(duration))
        ...
        if action == "scroll":
            ...
            y2 = y + distance if params.get("direction", "down") == "up" else y - distance
            await self._checked("shell", "input", "swipe", str(x), str(y), str(x), str(y2), "350")
            return {"direction": params.get("direction", "down"), "amount": amount}
        if action == "type":
    ...
    async def viewer(self) -> dict[str, Any]:
        ...
        arguments = ["--serial", str(self.config["serial"])] if self.config.get("serial") else []
